...
- AC-11: 游戏结束后收到 `game_over` 消息，验证：`winnerCamp` 为 `"landlord_camp"` 或 `"civilian_camp"`，`scores` 为 Record 且包含本玩家 sessionId，`Object.keys(scores).length === 5`（`multiplier`/`breakdown` 不在广播中，仅写 DB，不验证）
